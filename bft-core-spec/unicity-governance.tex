...
By \emph{governance}\index{governance}, we mean the operational management and re-configuration of a dynamic Unicity Platform instance. Governance processes manage the validators and other nodes, manage the lifecycle of aggregation clusters and govern sharding, manage incentives to the participants, and manage updates to the system.
...
\subsection{How a Validator joins a Cluster}

Validators rely on configuration agents for performing any steps involving user-level communication with other aggregation clusters and shards. The process works like this:

\begin{itemize}
	\item Configuration Agent detects a new Change Record it is involved with, learns the aggregation cluster identifier and shard number
...
The BFT Core plays active role in enforcing and synchronizing the Governance. It initiates epoch changes and accepts UC Requests only from valid members of aggregation clusters/shards. In order to do so, it must have access to authentic change requests.
...
In the Proof of Authority mechanism, a permissioned, \emph{authorized} entity provides the governance information by sending signed transactions to the governance aggregation cluster.
...
\begin{description}
	...
	\item[Liquid Staking] is a mechanism where otherwise locked stake can be used for other purposes, for example used for staking in multiple aggregation clusters. This allows stakers to achieve better yield at the cost of higher risks.
...
The process produces change records (called Validator Assignment Records), which assign validators to specific aggregation clusters and shards, as shown in Table~\ref{ta:var}.
...
\begin{table}[!htbp]
	\begin{center}
		...
		\begin{tabular}{|p{0.5cm}|p{8cm}|p{1.7cm}|p{3.5cm}|}\hline
			...
			1. & Cluster Identifier       & $\beta$       & $\mathbb{P}$                \\\hline
...
If a shard or entire aggregation cluster stops working at the start of an epoch, the corresponding change record will have empty values for the validator identifiers and stakes as well as for the quorum size ($\{\nu, b_\nu\}_e = \bot \land k_e = \bot$). At all other times the values must not be empty.
...
Hash of Previous Record may be implemented inexplicitly if the underlying base aggregation cluster provides hash linking of records.


\section{Aggregation Layer Cluster Lifecycle Management}

This process creates, modifies and deletes aggregation clusters. The output is Cluster Update Record with System Description Records and the configuration of consensus layer, as illustrated by Table~\ref{ta:pcr}.

\begin{table}[!htbp]
	\begin{center}
		\caption{Change Record: Cluster Update}\label{ta:pcr}
		\begin{tabular}{|p{0.6cm}|p{8cm}|p{1.7cm}|p{3.5cm}|}\hline
			No & Field                 & Notation & Type \\\hline\hline
			1. & Network Identifier    & $\alpha$ & $\mathbb{A}$       \\\hline
			2. & Aggregation Cluster Identifier  & $\beta$  & $\mathbb{P}$      \\\hline
			3. & Aggregation Cluster Description & $\mathcal{PD}[\beta]$ & $\mathbb{PD}$ \\\hline % includes sharding scheme
			...
			7. & Cluster Size          & $k$      & $\uint{32}$       \\\hline
...
This process creates and updates Sharding Schemes for aggregation clusters. The switch to a new sharding scheme is executed at epoch change; see Table~\ref{ta:ssu}.
...
\begin{table}[!htbp]
	\begin{center}
		...
		\begin{tabular}{|p{0.5cm}|p{8cm}|p{1.7cm}|p{3.5cm}|}\hline
			No & Field                      & Notation       & Type          \\\hline\hline
			1. & Network Identifier         & $\alpha$       & $\mathbb{A}$  \\\hline
			2. & Cluster Identifier       & $\beta$        & $\mathbb{P}$  \\\hline
