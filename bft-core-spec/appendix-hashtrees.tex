...
\textbf{Input}:
\begin{enumerate}
	\item $\mathsf{sid} \in \hashtype$ -- state identifier (key)
	\item $h_{\mathsf{tx}} \in \hashtype$ -- expected transaction hash (value)
...
\textbf{Computation}:
\begin{algorithmic}
	\Function{smt\_verify\_inclusion}{$\mathsf{sid}, h_{\mathsf{tx}}, C^\mathsf{inc}, h_{\mathsf{root}}$}
		\If {$d_1 \neq h_{\mathsf{tx}}$}
			\State \Return \FALSE \Comment{Leaf data mismatch}
		\EndIf
...
To prove that a key-value pair $(\mathsf{sid}, h_{\mathsf{tx}})$ exists in an SMT:
...
\begin{itemize}
	...
	\begin{enumerate}
		\item $\Call{smt\_verify\_inclusion}{\mathsf{sid}, h_{\mathsf{tx}}, C^\mathsf{inc}, h_{\mathsf{root}}} = \TRUE$
...
To prove that a state identifier $\mathsf{sid}_q$ does NOT exist in an SMT:

\begin{itemize}
	\item \textbf{Generation}: Find the position where $\mathsf{sid}_q$ would be located if it existed. Generate a certificate $C^\mathsf{exc} = \langle (p_1, d_1), \ldots, (p_n, d_n) \rangle$ showing this position.
	\item \textbf{Verification}: Check that:
	\begin{enumerate}
		\item The certificate reconstructs the correct root: \\
		      $\Call{smt\_verify\_inclusion}{\mathsf{sid}_p, d_1, C^\mathsf{exc}, h_{\mathsf{root}}} = \TRUE$ \\
		      where $\mathsf{sid}_p = p_1 \| \ldots \| p_n$ is the path from the certificate
		\item The position is either:
		\begin{itemize}
			\item Empty: $d_1 = \bot$ (no leaf exists at this location), OR
			\item Contains a different key: $\mathsf{sid}_p \neq \mathsf{sid}_q$ but shares a common prefix with $\mathsf{sid}_q$ (proving the queried key is absent)
		\end{itemize}
	\end{enumerate}
\end{itemize}

The exclusion proof shows the branch where $\mathsf{sid}_q$ would be inserted if it existed, cryptographically proving its absence from the current tree state.
