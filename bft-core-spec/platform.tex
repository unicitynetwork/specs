...
\begin{description}
\item[Consensus Layer] PoW chain and BFT Core. Provides token minting, decentralized agreement and finality through Byzantine Fault Tolerant consensus. This layer verifies the integrity of the Aggregation Layer's state transitions and serves as the root of trust for the entire system. The BFT Core certifies SMT root hashes submitted by aggregation clusters.

\item[Aggregation Layer] Maintains a global append-only registry of spent token states using sharded Sparse Merkle Trees (SMT). Each cluster within this layer processes state certification requests, provides inclusion proofs, and uses consistency proofs to enable trustless verification. The layer is organized into clusters for service design flexibility, with each cluster potentially divided into shards for horizontal scalability.
...
\chapter{Framework Data Structures}
...
In practice, there will be several instances of Unicity networks, each consisting of its own BFT Core and its own set of clusters. Each such network will have an identifier $\alpha$\index{$\alpha$ (network identifier)}:
...
\section{Clusters and Shards}

Unicity network's aggregation layer consists of multiple \emph{clusters}\index{cluster}. Each cluster is identified by a cluster identifier $\beta \in \mathbb{C}$\index{$\beta$ (cluster identifier)}\index{$\mathbb{C}$ (cluster identifier type)}. Each cluster runs an unicity proving service instance, or some other utility service instance\footnote{Roughly, this is similar to Layer 1, Layer 2 division of Ethereum type blockchains}.

A cluster may be divided into one or more \emph{shards}\index{shard} according to a sharding scheme (Sec.~\ref{sec:sharding}). Each shard is implemented by one or more \emph{aggregation layer nodes}\index{aggregation layer node} that maintain the SMT for their portion of the key space.

\section{Cluster Type and Cluster Type Descriptor}

Every aggregation cluster in the Unicity Framework with cluster identifier $\beta$ has a type that is either:
\begin{enumerate}
\item a \emph{standard type} (Unicity Tree), or
\item a \emph{defined type} that is described as a data structure of type $\mathbb{SD}$.
\end{enumerate}

\subsection{Cluster Type Descriptor}

A \emph{cluster type descriptor} is a tuple $\mathcal{SD} = (\mathsf{hf}, \mathbb{CP}, V_{CP}, \gamma_{CP}) \in \mathbb{SD}$, where:
\begin{itemize}
...
\subsection{Standard Cluster Types}

\textbf{Unicity SMT} ($\mathsf{st} = 1$) -- Sparse Merkle Tree based unicity prover

\textbf{EVM} ($\mathsf{st} = 2$) -- Enshrined EVM runtime

\subsection{Cluster Description Record}

A \emph{cluster description record}\index{$\mathcal{CD}$ (cluster description)}\index{cluster description} is a tuple $\mathcal{CD} = (\alpha, \beta, \mathsf{st}, \mathcal{SD}, \mathcal{SH}) \in \mathbb{CD}$, where:
\begin{itemize}
\item $\alpha \in \mathbb{A}$ is the network identifier;
\item $\beta \in \mathbb{C}$ is the cluster identifier;
...
For standard type clusters, the field $\mathcal{CD}.\mathcal{SD}$ does not exist. In this case, the notation $\mathcal{CD}.x$ will mean the constant value of $x$ of the standard type $\mathsf{st}$.

\section{Unit Type Identifiers}

Each transaction system has a defined list of unit types that it supports. Each unit type has an identifier of type $\mathbb{IT} = \uint{\mathsf{tidlen}}$. The identifier values $16 \ldots 31$ are reserved for common unit types supported across many transaction systems and should not be assigned to unit types specific to just one transaction system.

\section{Unicity Service Request}

A \emph{Unicity Service Request}\index{$Q$ (unicity service request)} is a request to the Unicity Service for state transition certification. It is a tuple $Q = (\mathsf{pk}, h_{\mathsf{st}}, h_{\mathsf{tx}}, \sigma)$, where:
\begin{itemize}
	\item $\mathsf{pk}$ -- current owner's public key
	\item $h_{\mathsf{st}} \in \hashtype$ -- current state hash
	\item $h_{\mathsf{tx}} \in \hashtype$ -- transaction data hash
	\item $\sigma$ -- owner's digital signature authorizing the state transition
\end{itemize}

The Unicity Service processes the request by:
\begin{enumerate}
	\item Verifying the digital signature: $\mathsf{Verify}_\mathsf{pk}(H(h_{\mathsf{st}}, h_{\mathsf{tx}}), \sigma) = 1$
	\item Computing the state identifier: $\mathsf{sid} = H(\mathsf{pk}, h_{\mathsf{st}})$
	\item Checking uniqueness: the state has not been spent before (i.e., $\mathsf{sid}$ does not already exist in the SMT)
	\item If all checks pass: setting the SMT leaf $(\mathsf{sid}, h_{\mathsf{tx}})$ in the tree
\end{enumerate}
...
Given a Unicity Service Request $Q = (\mathsf{pk}, h_{\mathsf{st}}, h_{\mathsf{tx}}, \sigma)$, the aggregation layer node performs the following checks:

\begin{enumerate}
	\item \textbf{Signature verification}: Verify that the signature is valid:
	\[
	\mathsf{Verify}_\mathsf{pk}(H(h_{\mathsf{st}}, h_{\mathsf{tx}}), \sigma) = 1
	\]

	\item \textbf{Uniqueness check}: Compute the state identifier and check that it does not already exist as a leaf in the SMT:
	\[
	\mathsf{sid} = H(\mathsf{pk}, h_{\mathsf{st}})
	\]
...
\[
\text{Set SMT leaf: } (\mathsf{sid}, h_{\mathsf{tx}})
\]
This updates the SMT by inserting the new leaf.

If either check fails, the request is rejected:
\begin{itemize}
	\item If signature verification fails: the request is invalid and must be rejected
...
\emph{Certificates} are compact proofs of inclusion (or sometimes uniqueness, or exclusion) of some data item in an authenticated data structure. Certificates may be chained. Combining certificates, it is possible to put together \emph{proofs} (Sec.~\ref{se:proofs}) proving e.g. execution of a transaction, or existence of a unit state.
...
See Fig.~\ref{fi:inc-proof} for the Inclusion Proof structure and Fig.~\ref{fi:uc} for the Unicity Certificate. The Inclusion Proof contains an SMT leaf $L = (\mathsf{sid}, h_{\mathsf{tx}})$ where $\mathsf{sid} = H(\mathsf{pk}, h_{\mathsf{st}})$ is a state identifier and $h_{\mathsf{tx}}$ is a transaction hash. The Inclusion Certificate $C^\mathsf{inc}$ connects the leaf to the SMT root hash $h$ in the Input Record. The Unicity Certificate then chains through the shard tree, unicity tree, and unicity seal to the global root of trust: Unicity Trust Base $\mathcal{T}$.


\subsection{Sparse Merkle Tree}\label{sec:smt}

\index{sparse merkle tree}\index{SMT}
The aggregation layer maintains a \emph{Sparse Merkle Tree} (SMT) in compressed paths form. The SMT maps state identifiers to transaction hashes and provides compact inclusion proofs\footnote{This specification does not cover non-inclusion proofs and consistency (no-deletion) proofs}.

\subsubsection{Path Compression}

The SMT retains only non-empty leaves and only parent nodes with two child nodes explicitly (the root node may have zero, one, or two children). Each edge in the tree is labeled with a path description (a bit-string).

\begin{itemize}
	\item \textbf{Leaf node hash}: $h = H(p, d)$ where $p$ is the path label to the parent and $d$ is the leaf data
	\item \textbf{Branch node hash}: $h = H(p, h_L, h_R)$ where $p$ is the path label to the parent, $h_L$ is the left child hash, and $h_R$ is the right child hash
	\item \textbf{Root node}: Uses empty path $\epsilon$ in hash computation
\end{itemize}
...
\begin{itemize}
	\item \textbf{Key}: State identifier $\mathsf{sid} = H(\mathsf{pk}, h_{\mathsf{st}})$ where:
	\begin{itemize}
		\item $\mathsf{pk}$ -- public key of an unicity token owner
		\item $h_{\mathsf{st}}$ -- state hash representing the current state of a token
	\end{itemize}
	\item \textbf{Value}: Transaction data hash $h_{\mathsf{tx}} = H(D_{\mathsf{tx}})$ of type $\hashtype$, where $D_{\mathsf{tx}}$ is the transaction data containing:
	\begin{itemize}
		\item Recipient's public key $\mathsf{pk}'$
		\item Random blinding mask $x$ for privacy and state evolution
		\item Recipient's auxiliary data
		\item Additional optional transaction-specific data, e.g. persistent token attributes if this is a mint transaction
	\end{itemize}
\end{itemize}

The SMT effectively maintains a mapping $\mathsf{sid} \mapsto h_{\mathsf{tx}}$ that proves a state transition from state identifier $\mathsf{sid}$ was authorized via transaction with hash $h_{\mathsf{tx}}$. This prevents double-spending: once a state identifier is registered in the SMT with a transaction hash, that state cannot be spent again.
...
The concatenated path $p_1 \| p_2 \| \ldots \| p_n$ equals the full key $\mathsf{sid}$. The respective value $d_1$ records $h_\mathsf{st}$.

\subsubsection{Verification: $\mathsf{VerifyInclusionCert}$}\label{VerifyInclusionCert}\index{functions!VerifyInclusionCert}

\textbf{Input}:
\begin{enumerate}
	\item $\mathsf{sid}$ -- state identifier (key) of type $\hashtype$
	\item $h_{\mathsf{tx}}$ -- transaction hash (value) of type $\hashtype$
	\item $C^\mathsf{inc} = \langle (p_1, d_1), (p_2, d_2), \ldots, (p_n, d_n) \rangle$ -- Inclusion Certificate
	\item $h_{\mathsf{root}}$ -- expected root hash of type $\hashtype$
\end{enumerate}
...
\textbf{Computation}:
\begin{algorithmic}[0]
	\If {$d_1 \neq h_{\mathsf{tx}}$} \Return \FALSE \EndIf \Comment{Verify leaf data}
	\If {$p_1 \| p_2 \| \ldots \| p_n \neq \mathsf{sid}$} \Return \FALSE \EndIf \Comment{Verify path}
	\State $h \gets H(p_1, d_1)$ \Comment{Hash the leaf node}
	\For {$i \gets 2$ to $n$}
		\State $b \gets$ rightmost bit of $p_{i-1}$ \Comment{Determine position}
		\If {$b = 0$}
			\State $h \gets H(p_i, h, d_i)$ \Comment{Current node is left child}
		\Else
			\State $h \gets H(p_i, d_i, h)$ \Comment{Current node is right child}
		\EndIf
	\EndFor
...
\begin{enumerate}
	\item $\beta$ -- cluster identifier of type $\mathbb{C}$
	\item $\mathsf{dhash}$ -- cluster description hash of type $\hashtype$
	\item $(\beta_2, h_2), \ldots, (\beta_\ell, h_\ell)$ -- a sequence of cluster identifier and sibling hash pairs
\end{enumerate}
...
\begin{enumerate}
	\item $\beta$ -- cluster identifier of type $\mathbb{C}$
	\item $\mathcal{C}$ -- set of cluster identifiers of type $\mathbb{C}$
	\item $\mathcal{CD}$ -- cluster description of type $\mathbb{CD}[\mathcal{C}]$
...
\textbf{Computation}:
\begin{algorithmic}
	\State $n \gets \len{\mathcal{C}}$ \Comment{Number of clusters}
...
An SMT leaf $L$ is a tuple $L = (\mathsf{sid}, h_{\mathsf{tx}})$, where:
\begin{itemize}
	\item $\mathsf{sid} = H(\mathsf{pk}, h_{\mathsf{st}})$ -- state identifier, computed as the hash of token owner's public key and state hash
	\item $h_{\mathsf{tx}}$ -- transaction data hash
\end{itemize}
...
\begin{algorithmic}[0]
	...
	\If {$\neg \mathsf{VerifyInclusionCert}(\mathsf{sid}, h_{\mathsf{tx}}, C^\mathsf{inc}, h)$} \Return \FALSE \EndIf
...
The consistency proof enables the BFT Core to verify the integrity of the Aggregation Layer's operations in a trustless manner. The BFT Core can cryptographically verify that:
\begin{enumerate}
	\item All newly inserted leaves corresponded to empty (unset) positions before the update
	\item After the update, the new leaves are correctly placed at their designated positions
	\item No other changes were made to the SMT
\end{enumerate}

\subsubsection{Structure}

For a batch of state transitions processed in a single round, the consistency proof $\pi_{CP}$ of type $\mathbb{CP}$ is a tuple:
\[
\pi_{CP} = (B, \Pi)
\]
where:
...
The consistency proof is generated by the aggregation layer node as follows:
...
% full algorithm (not separated from tree calculation)
% https://github.com/unicitynetwork/zkvm-ndsmt/blob/b3ee2ab20b3803100459a4b36b6cef405d175865/script/ndsmt.py#L42

\subsubsection{Verification: $\mathsf{VerifyConsistencyProof}$}\label{VerifyConsistencyProof}\index{functions!VerifyConsistencyProof}
...
\begin{algorithmic}[0]
	...
	\State $h_\emptyset \gets \Call{ComputeTreeRoot}{\Pi, B_\emptyset}$ \Comment{Compute root with empty leaves}
	\If {$h_\emptyset \neq h'$} \Return \FALSE \EndIf \Comment{Verify previous state}
	\State $h_B \gets \Call{ComputeTreeRoot}{\Pi, B}$ \Comment{Compute root with actual leaves}
...
\noindent\textbf{BFT Core Validation:} When the BFT Core receives a Certification Request with consistency proof $\pi_{CP}$ (Sec.~\ref{certification-request}), it must verify that the computed root hashes match the values in the Input Record:
\begin{enumerate}
	\item Extract $h'$ and $h$ from $\mathcal{IR}$ in the Certification Request
	\item Verify $\mathsf{VerifyConsistencyProof}(\pi_{CP}, \mathcal{IR}.h', \mathcal{IR}.h) = \TRUE$
	\item Verify $|B| = \ell_B$ (batch size matches the claimed number of additions)
\end{enumerate}

This ensures the consistency proof is bound to the specific Input Record being certified.

\medskip

\noindent The auxiliary function $\mathsf{ComputeTreeRoot}(\Pi, B)$ computes the SMT root hash from the batch $B$ and proof siblings $\Pi$:
...
A valid consistency proof guarantees:
\begin{enumerate}
	\item \textbf{Append-only property}: No existing leaves were deleted or modified
	\item \textbf{Correctness}: All new leaves in batch $B$ were correctly inserted at positions determined by their state identifiers
	\item \textbf{Completeness}: The state transition from $h'$ to $h$ includes exactly the leaves in $B$ and no others
\end{enumerate}

\subsubsection{ZK-Compressed Consistency Proofs}\label{sec:zk-consistency-proof}
...
\textbf{ZK Proof Structure:}

A ZK-compressed consistency proof proves successful execution of the $\mathsf{VerifyConsistencyProof}$ algorithm with the following parameters:
...
\textbf{BFT Core Validation:} When receiving a ZK-compressed proof:
\begin{enumerate}
	\item Verify that the proof is proving the right statement (hash-based consistency proof verification algorithm)
	\item Verify the ZK proof, get authentic public inputs $h'$, $h$, and $j$
...
\emph{Shard input record} ($IR$)\index{$IR$ (input record)} of a shard of a cluster (of type $\mathbb{IR}$) is a tuple $(n, e, h', h, t)$, where:
\begin{enumerate}
...
There is one Statistical Record of current epoch where the values are being updated, reflecting the current state since the beginning of the epoch; and one invariant Statistical Record of the preceding epoch of every shard of every public cluster.
...
There is one Technical Record for every shard of every cluster, providing synchronization for the next round.
...
For a cluster $\beta$, let $\mathcal{SH}_\beta = \mathcal{CD}[\beta].\mathcal{SH}$, and
...
Shard tree\index{shard tree} for a cluster $\beta$ is a function $\chi_\beta \colon \overline{\mathcal{SH}_\beta} \to \hashtype$ such that:
\begin{enumerate}
...
Unicity Tree is an indexed Merkle tree that aggregates the state of all clusters in the network.
...
\begin{enumerate}
	...
	\item $\mathcal{C}$ -- set of cluster identifiers, with elements of type $\mathbb{C}$
	\item $\mathcal{CD}$ -- cluster descriptions of type $\mathbb{CD}[\mathbb{C}]$
...
During every round, the BFT Core receives certification requests from the shards of clusters
$\beta \in \mathcal{C}$.
...
\item $\mathsf{CR}.\beta\in \mathcal{C}$ and $\sigma\in \mathcal{CD}[\mathsf{CR}.\beta].\mathcal{SH}$ -- request came from a legitimate shard of a registered cluster
...
\item $\mathcal{CD}[\beta].\gamma_{CP}(\mathsf{CR}.\mathcal{IR}, \mathsf{CR}.\pi_{CP})$ -- consistency proof verification succeeds; consistency proof matches the IR.
...
\begin{enumerate}
	...
	\item For every registered cluster $\beta\in\mathcal{C}$, the collected certification requests $\mathsf{CR}_{\beta,\sigma}$ are converted to a temporary data structure $\mathcal{IRT}_\beta$ of type $(\mathbb{IR},\hashtype)[\mathcal{SH}_\beta]$, where $\mathcal{SH}_\beta=\mathcal{CD}[\beta].\mathcal{SH}$,
